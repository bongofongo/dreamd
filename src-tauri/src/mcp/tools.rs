...
//!
//! Three read tools, two write tools, and none of them writes a file byte.
//! Tenet 1 holds: dreamd exposes reading and annotation *state*; changing a
//! document stays with the agent's own `Edit`/`Write`.
//!
...
pub fn dispatch(store: &mut Store, root: &Path, call: &ToolCall) -> ToolResult {
...
/// a dead end.
pub fn call(store: &mut Store, root: &Path, call: &ToolCall) -> Value {
    match dispatch(store, root, call) {
        Ok(value) => {
...
mod tests {
    ...
    fn run(store: &mut Store, name: &str, args: Value) -> ToolResult {
        dispatch(store, Path::new("/repo"), &ToolCall::new(name, args))
    }
