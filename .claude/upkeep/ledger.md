...
| 13 | `ui-agent` | — | skipped ten nights running, 2026-08-18 through 2026-08-27 — the same three upkeep PRs are open every night (#24, #25, #26), at the backlog cap. Still not one comment, review or push since each was opened: #24 is twelve days old, #25 eleven, #26 ten. Re-checked tonight against the current tip: all three still **merge clean** into `main`, and no two of them touch the same file — #24 is one findings markdown, #25 is `CLAUDE.md` + four `src-tauri/` files, #26 is `packaging/` + the three workflows — so they can be merged in any order, or any one alone. `main` has moved only on this ledger since they were branched. Merging any one of them unblocks the rotation, and nothing else will. No sweep run any night, so **last swept** stays empty and `ui-agent` is still the area due | — |
| 14 | `perf-harness` | — | — | — |
| 15 | `ui-style` | — | — | — |
