...
| 13 | `ui-agent` | — | skipped nine nights running, 2026-08-18 through 2026-08-26 — the same three upkeep PRs are open every night (#24, #25, #26), at the backlog cap. Still not one comment, review or push since each was opened: #24 is eleven days old, #25 ten, #26 nine. All three are **green on every CI check** and none can conflict — `main` has moved only on this file since they were branched, and none of them touches it — so merging any one of them unblocks the rotation, and nothing else will. No sweep run any night, so **last swept** stays empty and `ui-agent` is still the area due | — |
| 14 | `perf-harness` | — | — | — |
| 15 | `ui-style` | — | — | — |
